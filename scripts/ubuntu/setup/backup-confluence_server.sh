#!/bin/bash
#############################################################################################
# This script is using for backup the conlufence server on 233 server                       #    
# @Author : Fredy Zhang                                                                     #
...
echo " Usage: $0 <options>
	...
	Mandatory options:
	--install | -i           : default no, set yes to backup the package to server. 
...
setup_app()
{
	...
	cd $cwd && ./expect/expect_confluence_server.sh $SERVER_IP $REMOTE_IP
