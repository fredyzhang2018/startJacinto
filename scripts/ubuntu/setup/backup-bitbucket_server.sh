#!/bin/bash
#############################################################################################
# This script is using for backup the bitcketbut server on 233 server                       #    
# @Author : Fredy Zhang                                                                     #
...
SETUP_YES_NO="no"
CPU_NUM=`nproc`
SERVER_IP="10.85.130.233"
REMOTE_IP="10.85.130.156"
...
echo " Usage: $0 <options>
	...
	Mandatory options:
	--install | -i           : default no, set yes to backup.

	Optional options:
	--verbose | -v           : verbose print output.
	--version | -s           : set the version (Default : $VERSION )
	-p                       : back up to backup server

More details , please visit : no 
"
}
	...
	do 
		...
		case $args in
			...
			"--path" | "-p")
				...
				echo_log "- Step1: login and backup the server: $cwd "
				# ./expect/expect_bitbucket_server.sh SERVER_IP REMOTE_IP
				cd $cwd && ./expect/expect_bitbucket_server-remote.sh $SERVER_IP $REMOTE_IP
				echo_log "- Step1: login and backup the server. --- done"
				;;
			"--launch" | "-l")
				;;
			"*")
...
setup_app()
{
	...
	echo_log "- Step1: login and backup the server: $cwd "
	# ./expect/expect_bitbucket_server.sh SERVER_IP REMOTE_IP
	cd $cwd && ./expect/expect_bitbucket_server.sh $SERVER_IP $REMOTE_IP
	echo_log "- Step1: login and backup the server. --- done"
