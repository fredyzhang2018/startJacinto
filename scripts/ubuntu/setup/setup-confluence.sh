...
More details , please visit : https://github.com/teamatldocker/confluence
	install guide: https://confluence.atlassian.com/doc/confluence-installation-and-upgrade-guide-214864161.html
	docker is not accessed in the internal network. 
"
}
	...
	do 
		...
		case $args in
			...
			"--path" | "-p")
				...
				;;
			"--launch" | "-l")
				cd $REPO_INSTALL_PATH/confluence/atlassian-confluence-$VERSION/bin && ./start-confluence.sh 
				;;
			"*")
...
setup_app()
{
	...
	if [ -d $REPO_INSTALL_PATH ];then
		...
		if [ -d $REPO_INSTALL_PATH/confluence ];then
			if [ ! -d $REPO_INSTALL_PATH/confluence/atlassian-confluence-$VERSION ];then 
				tar -xzvf $SJ_PATH_DOWNLOAD/atlassian-confluence-$VERSION.tar.gz -C $REPO_INSTALL_PATH/confluence
				cd $REPO_INSTALL_PATH/confluence  && tar -xvf atlassian-confluence-$VERSION.tar
				cd $REPO_INSTALL_PATH && chown -R `whoami` $REPO_INSTALL_PATH/confluence
				cd $REPO_INSTALL_PATH && chmod -R u=rwx,go-rwx $REPO_INSTALL_PATH/confluence
			else 
				echo "- alread installed $REPO_INSTALL_PATH/confluence/atlassian-confluence-$VERSION"
			fi
		fi
	else 
	...
	cd $USER_HOME && mkdir -p confluence-home
	chown -R fredy  $USER_HOME/confluence-home
	chmod -R u=rwx,go-rwx $USER_HOME/confluence-home
	echo_log "- step3 : setup the home dictionary used for data" 
	cat $REPO_INSTALL_PATH/confluence/atlassian-confluence-$VERSION/confluence/WEB-INF/classes/confluence-init.properties | grep confluence.home
	sed -i "/^# confluence.home=c/c confluence.home=$USER_HOME/confluence-home" $REPO_INSTALL_PATH/confluence/atlassian-confluence-$VERSION/confluence/WEB-INF/classes/confluence-init.properties
