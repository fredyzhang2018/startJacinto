...
VERSION="3.11.3"
APP_DOWNLOAD_URL="https://github.com/protocolbuffers/protobuf.git" # github repo
...
More details , please visit : https://github.com/protocolbuffers/protobuf
"
}
...
setup_release_app()
{
	echo_log "[ $(date) ] --- ${FUNCNAME[0]}: args --- $#" 
	# setup the package. 
	if [ !  -d $APP_INSTALL_PATH/protobuf-$VERSION ];then
		cd $APP_INSTALL_PATH && wget $APP_DOWNLOAD_URL1
		cd $APP_INSTALL_PATH && tar -zxvf v$VERSION.tar.gz
	else 
...
setup_src_app()
{
	...
	cd $REPO_INSTALL_PATH/protobuf && git checkout v$VERSION
	cd $REPO_INSTALL_PATH/protobuf  && git log -1 | grep $VERSION
	cd $REPO_INSTALL_PATH/protobuf && git submodule update --init --recursive
	cd $REPO_INSTALL_PATH/protobuf && ./autogen.sh

	# #   # build
	echo_log "[ $(date) ] --- ${FUNCNAME[0]}: 3. build"
	cd $REPO_INSTALL_PATH/protobuf && ./configure CXXFLAGS=-fPIC --enable-shared=no LDFLAGS="-static"
...
update_packge_to_target_dictionary()
{
	echo_log "[ $(date) ] --- ${FUNCNAME[0]}: args --- $#" 
	# setup the package. 
	if [ !  -L $APP_INSTALL_PATH/protobuf-$VERSION ];then
		ln -s  $REPO_INSTALL_PATH/protobuf $APP_INSTALL_PATH/protobuf-$VERSION
	else 
