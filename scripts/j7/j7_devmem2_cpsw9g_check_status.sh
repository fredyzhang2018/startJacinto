...
CPSW_SS_SGMII_CONTROL_REG_j()
{
	...
	devmem_read $reg_temp
	echo "CPSW_SS_SGMII_CONTROL_REG_ $PORT_NUM ($reg_temp) = $reg_val  "
...
CPSW_SS_SGMII_STATUS_REG_j()
{
	...
	reg_temp=`printf "0x%08x" $(($PORT_NUM*0x100+$base_addr))`
	devmem_read $reg_temp
	echo "CPSW_SS_SGMII_STATUS_REG_ $PORT_NUM ($reg_temp) = $reg_val  "
	echo "	LOCK                  : "`check_field $reg_val 4  1 "0h SEDES PLL isn't locked" "1h SERDES PLL is locked. "`
	echo "	MR_AN_COMPLETE        : "`check_field $reg_val 2  1 "0h no AUTO Negotiation is not complete" "1h AUTO negotiation completed."`
	echo "	AN_EEROR              : "`check_field $reg_val 1  1 "0h no negotiation error " "1h AUTO negotiation error"`
