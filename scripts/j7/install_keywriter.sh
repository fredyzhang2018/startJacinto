...
launch()
{
	...
	then
		...
		echo "[ $(date) ] - 1 $PDK_PATH/`echo $KEYWRITER_NAME | cut -d / -f 6 | cut -d . -f 1` exist exist, please check >>>" 
	fi
	...
	then
		...
		# cd $PDK_PATH/packages/ti/build/ && make sciclient pmic sciclient_direct
